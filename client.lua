...
-- Khai báo lib từ ox_lib
local lib = exports.ox_lib and exports.ox_lib or nil
if not lib then
    print('^1Lỗi: ox_lib không được tải, các menu và thông báo sẽ không hoạt động!^7')
end
...
AddEventHandler('vehicle:upgrade', function(data)
    ...
    if not DoesEntityExist(vehicle) then 
        if lib then lib.notify({ title = 'Lỗi', description = 'Không có xe nào gần bạn!', type = 'error' }) end 
        return 
    end
...
AddEventHandler('vehicle:upgrade:apply', function(upgradeType, vehicleNetId)
    ...
    if DoesEntityExist(vehicle) then
        ...
        ESX.TriggerServerCallback('vehicle:getUpgradeStatus', function(status)
            currentStatus = status
        end, plate)
        while currentStatus == nil do Citizen.Wait(0) end
        ...
        if lib then lib.notify({ title = 'Thông báo', description = 'Trạng thái xe ' .. plate .. ': ' .. upgradeName .. ' cấp ' .. currentLevel, type = 'inform' }) end
        if currentLevel < maxLevel then
            ...
            if lib then lib.notify({ title = 'Thành công', description = 'Xe ' .. plate .. ' đã nâng cấp ' .. upgradeName .. ' lên cấp ' .. (currentLevel + 1), type = 'success' }) end
        else
            if lib then lib.notify({ title = 'Lỗi', description = 'Phương tiện đã đạt cấp tối đa cho ' .. upgradeName .. '!', type = 'error' }) end
        end
    end
end)
...
AddEventHandler('vehicle:paint:apply', function(vehicleNetId)
    ...
    if DoesEntityExist(vehicle) then
        ...
        -- Định nghĩa colorOptions ở scope ngoài cùng
        local colorOptions = {
            { label = 'Đen', args = 0 },
        ...
        local function showPrimaryPaintMenu(page)
            ...
            for i = startIndex, endIndex do 
                if colorOptions[i] then
                    table.insert(pageOptions, colorOptions[i]) 
                end
            end
            ...
            if lib then
                if not pageOptions or #pageOptions == 0 then
                    print('^1Lỗi: pageOptions trống trong showPrimaryPaintMenu, page = ' .. page .. '^7')
                    return
                end
                lib.registerContext({
                    ...
                    onSelect = function(selectedPrimary)
                        ...
                        elseif selectedPrimary.args == 'next' then showPrimaryPaintMenu(page + 1)
                            ...
                            local function showSecondaryPaintMenu(pageSec)
                                ...
                                for i = startIndexSec, endIndexSec do 
                                    if colorOptions[i] then
                                        table.insert(pageOptionsSec, colorOptions[i]) 
                                    end
                                end
                                ...
                                if not pageOptionsSec or #pageOptionsSec == 0 then
                                    print('^1Lỗi: pageOptionsSec trống trong showSecondaryPaintMenu, pageSec = ' .. pageSec .. ', startIndexSec = ' .. startIndexSec .. ', endIndexSec = ' .. endIndexSec .. '^7')
                                    return
                                end
...
AddEventHandler('vehicle:turbo:apply', function(vehicleNetId)
    ...
    if DoesEntityExist(vehicle) then
        ...
        while currentStatus == nil do Citizen.Wait(0) end
        if currentStatus.turbo == 1 then 
            if lib then lib.notify({ title = 'Lỗi', description = 'Xe ' .. plate .. ' đã có turbo!', type = 'error' }) end 
            return 
        end
        RequestAnimDict('mini@repair') while not HasAnimDictLoaded('mini@repair') do Citizen.Wait(100) end
        TaskPlayAnim(playerPed, 'mini@repair', 'fixing_a_ped', 8.0, -8.0, 5000, 0, 0, false, false, false)
        ...
        if lib then lib.notify({ title = 'Thành công', description = 'Xe ' .. plate .. ' đã được lắp turbo!', type = 'success' }) end
    end
end)
...
AddEventHandler('vehicle:reset:apply', function(resetType, vehicleNetId)
    ...
    if DoesEntityExist(vehicle) then
        ...
        if currentLevel == 0 or (resetType == 'horn' and currentLevel == -1) or (resetType == 'pearlescent' and currentLevel == -1) then
            if lib then lib.notify({ title = 'Lỗi', description = resetName .. ' của xe ' .. plate .. ' đã ở trạng thái mặc định!', type = 'error' }) end 
            return
        end
        if lib then
            lib.registerContext({
        ...
        if IsControlJustPressed(0, 38) then -- Phím E (mã 38)
            if not IsPedInAnyVehicle(playerPed, false) then
                ...
                if DoesEntityExist(vehicle) then
                    ...
                    local status = nil
                    ESX.TriggerServerCallback('vehicle:getUpgradeStatus', function(result)
                        status = result
                    end, plate)
                    while status == nil do Citizen.Wait(0) end
                    ...
                    if lib then
                        ...
                        print('^1Không thể hiển thị menu trạng thái xe vì ox_lib không hoạt động^7')
                    end
                end
            end
        end
